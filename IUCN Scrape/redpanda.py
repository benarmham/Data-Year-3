...
for species in root.findall('species'):
	...
	# find out if the location is native or possibly extinct etc
	d = re.findall('^(.*?):', location)
	for string in d:
		description = string		
		print description
		f.write(description + ',')
	...
	for string in countries:
		c = string	
		markers = re.findall('\s(.*?);', c)
		
		# remove extra detail that will confuse google (anything after a comma or bracket)
